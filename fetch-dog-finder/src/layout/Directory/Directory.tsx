...
import { generateTitle, getAgeRange, getSortOrder } from './helpers'
...
const Directory = () => {
	...
	const updateLocation: FieldChangeHandler = (event) => {
		...
		setGeolocation(state => ({ ...state, ...entry }))
		resetPagination()
	}
			...
			<aside className={ sidebarClasses } onClick={ toggleSidebar }>
				<nav className="sidebar">
					<Header className="sidebar__header">
						<h2>Filter & Sort</h2>
						{ hasDefaultValues && (
							<Button onClick={ resetSortFilters } variant="text">
								Reset All
							</Button>
						) }
			...
			<main className="container">
				...
				<section className="directory">
					{ !dogs?.length && (
						<p>
							{ dogs === null
								? <>Fetching our network of paw-fect dogs...</>
								: <i>No results matching your criteria. Consider broadening your search.</i>
							}
						</p>
					) }
					{ dogs?.map(dog => (
